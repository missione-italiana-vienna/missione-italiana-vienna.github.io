  ...
  if (type_of_controller === "home") {
    ...
    if (type_of_controller === "calendar") {
      if ($route.current.params.hasOwnProperty("show_all_dates") && $route.current.params.show_all_dates === "yes") {
        sharedProperties.generate_html_with_all_events("future", "", "show_all_dates");
      }
      else {
        sharedProperties.generate_html_with_all_events("future", "", "show_only_one_year");
      }
...
function($rootScope, $sce, $http, $q, $httpParamSerializerJQLike) {
  ...
  function generate_html_with_all_events_after_load(input_string, events, additional_parameters) {
    ...
    for (i = 0; i < events.length; ++i) {
      ...
      if (input_string == "future") {
        ...
        if (start_year < year) { // this means that we are switching year in the list of events in the calender
  ...
  function add_news_blocks(id_first_news, id_last_news) {
    ...
    for (var i = id_last_news; i >= id_first_news; i--) {
      ...
      $("#container_of_news_" + i).html("Loading...").load("https://mcivienna.org/home/notizie/" + name_of_file + ".html");
    }
  }
  ...
  function create_popup_links(popups) {
    ...
    for (var i = 0; i < popups.length; i++) {
      if (popups[i].first_date_to_show <= formatted_date && popups[i].last_date_to_show >= formatted_date) {
        chosen_popups.push({
          "text": popups[i].text,
          "link": popups[i].link
        });
    ...
    if (!popups_already_created && chosen_popups.length > 0) {
      ...
      for (var i = 0; i < chosen_popups.length; i++) {
        ...
        create_single_popup_link(chosen_popups[i].text, chosen_popups[i].link, chosen_popups[i].action, i);
      }
  ...
  return {
    ...
    set_links_per_liturgia_del_giorno: function() {
      function data_liturgia_del_giorno(my_string) {
        ...
        if (selected_link) {
          ...
          selected_link.href = "https://www.chiesacattolica.it/liturgia-del-giorno/?data-liturgia=" + yyyy + mm + dd;
        }
      }
    ...
    generate_html_with_all_events: function(input_string, year, additional_parameters) {
      var fetch_url;
      if (input_string === "future") {
      ...
      if (fetch_url !== "") {
        ...
        xmlhttp.onreadystatechange = function() {
          if (this.readyState == 4 && this.status == 200) {
            ...
            generate_html_with_all_events_after_load(input_string, events, additional_parameters);
            // TO DO: handle the case when this is not parsable/cannot be loaded.
          }
        };
